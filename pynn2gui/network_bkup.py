...
class Network(network.Network):
    ...
    def _cell_desc(self,id,cell,data):
        cell.set('id','%s' %(id))
        cell.set('value','%s' % data['population']['label'])
    ...
    def _data_update(self,data,pop):
        for parameter_name in pop.celltype.default_parameters:
            data['population']['size'] = pop.size
            data['population']['celltype'] = pop.celltype.__class__.__name__
            data['population']['label'] = pop.label
            data['population']['name_value'] = pop.label
            match = ''
            max_score = 0.
            score = 0.
            for d in data['population']:
